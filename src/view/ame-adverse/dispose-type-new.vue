    ...
    <Modal
      ...
      <div>
        <Form ref="modify_userAddFormItem" :model="userAddFormItem" :rules="ruleUserAddFormItem" :label-width="80">
          <FormItem label="事件类型" prop="name">
            <Input v-model="userAddFormItem.name" disabled style="width: 250px" placeholder="请输入事件类型名称"></Input>
          </FormItem>
          <FormItem label="描述" prop="textarea">
            <Input v-model="userAddFormItem.textarea" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入事件类型描述"></Input>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    modify (data) {
      ...
      this.userAddFormItem.eventTypeId = data.id
    },
    append (data) {
      if (data === 0) {
        // 添加最高父级
    ...
    remove (root, node, data) {
      if (window.confirm(`确认删除-${data.title}-吗？请注意:该操作会将该层级中的所有子级全部删数。`)) {
        ...
        this.axios.get('/eventType/delAll/', { params: id })
          .then((response) => {
            this.geteventTypeDataFromService()
          })
          .catch((error) => {
